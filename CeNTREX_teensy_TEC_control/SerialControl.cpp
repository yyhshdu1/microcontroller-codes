...
SerialControl::SerialControl(Stream *serial, MTD415State *mtd415_state, PhysicalControl *physica1, DisplayControl *display_ctrl){
  _serial = (Stream *)serial;
  _mtd415_state = (MTD415State *)mtd415_state;
  _physical = (PhysicalControl *)physica1;
...
void SerialControl::_handleWrite(){
  ...
  switch(cmd){
    ...
    case 'e':
      this->_stringToArray(_response.substring(1,len-1), _iarray);
      _physical->setEnable(_iarray);
      break;
    case 's':
      _display_ctrl->display_state = _response.substring(1,len-1).toInt();
      break;
    default:
